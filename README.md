...
如果你是在 Windows 电脑上使用这个插件，并且没有安装 Git，可以直接双击运行根目录下的：

- `update.bat`

脚本会自动执行以下操作：

- 优先使用项目目录下的 `tools/git/cmd/git.exe`
- 如果项目目录和系统里都没有 Git，会自动下载官方 MinGit 到 `tools/git`
- 从 Gitee 拉取 `main` 分支最新代码到临时目录
- 自动覆盖当前插件目录中的文件

更新完成后，请重新打开 `chrome://extensions/`，然后点击一次扩展的“重新加载”。

如果你想提前把 Git 放进项目目录，也可以先双击：
